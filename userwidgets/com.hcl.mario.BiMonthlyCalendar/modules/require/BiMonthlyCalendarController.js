...
//   const SKIN_FLX_DAY_SELECTED = 'skinFlxDaySelected';
  ...
  return {
    ...
    setMonth({month, year}){
      ...
      this.setBottomMonth({month: month < 12 ? month + 1 : 1, year: month < 12 ? year : year + 1});
    ...
    selectDay(mcd){
      ...
      currentMcds.forEach((currentMcd) => {
        ...
        if(currentMcd.bottom){
          currentMcd.mcd.skinDay = (currentMcd.mcd.month !== this.view.monthSelectorBottom.month) ? SKIN_DAY_OTHER_MONTH : 
          (calendarUtils.isHoliday({day: currentMcd.mcd.day, month: currentMcd.mcd.month, year: currentMcd.mcd.year}) ? SKIN_DAY_HOLIDAY : SKIN_DAY);
        } else {
          currentMcd.mcd.skinDay = (currentMcd.mcd.month !== this.view.monthSelector.month) ? SKIN_DAY_OTHER_MONTH : 
          (calendarUtils.isHoliday({day: currentMcd.mcd.day, month: currentMcd.mcd.month, year: currentMcd.mcd.year}) ? SKIN_DAY_HOLIDAY : SKIN_DAY);
        }
      });
    ...
    findDay({day, month}){
      const ret = this.findDays({day, month});
